...
class CreateDpfSolicitationInternalBloc extends Bloc<CreateDpfSolicitationInternalEvent, CreateDpfSolicitationInternalState> {

  final CreateDpfSolicitationInternalRepository repository;
  CreateDpfSolicitationInternalBloc(this.repository) : super(CreateDpfSolicitationInternalInitial()) {
  ...
  Future<void> createDpfSolicitInterBloc(
    ...
    try{
      ...
      final response = await repository.createDpfSoliciInternal(token, event.phoneNumber, event.contextData, event.identityCardNumber, location, event.interesUpdate, event.amount, event.amountFinalUpdate, event.termUpdate, event.rateUpdate, event.email, event.names, event.term, event.idOfficeDPF, event.debitAccountCode, idPerson, event.ipNumber, event.idAccount, idUser, idWebPerson, event.isEmployee);
      ...
      if (error.message == "api_logic_error") {
        ...
        emit(CreateDpfSolicitationInternalError("Ocurrió un error, no tiene internet"));
      } else {
        emit(CreateDpfSolicitationInternalError(error.message));
      }
    }
